  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-white shadow-md">
      <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
        <button type="button" onClick={() => setMenuOpen(!menuOpen)} className="md:hidden w-12 h-12 flex flex-col justify-center items-center bg-transparent border-none outline-none cursor-pointer" style={{ WebkitTapHighlightColor: "transparent", touchAction: "manipulation" }}>
          <i className={`bi ${menuOpen ? "bi-x-lg" : "bi-list"} text-2xl text-gray-700`} />
        </button>
        <Link href="/#hero" scroll={true} className="text-2xl font-bold text-orange-500 tracking-tight font-[var(--font-playfair)]">
